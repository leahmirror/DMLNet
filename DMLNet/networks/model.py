...
from networks import resnet,resnet1
...
class MainNet(nn.Module):
    def __init__(self, proposalN, num_classes, channels):
        ...
        self.pretrained_model1 = resnet1.resnet50(pretrained=True, pth_path=pretrain_path)
    ...
    def forward(self, x, epoch, batch_idx, status='test', DEVICE='cuda'):
        ...
        #batch_size, channel_size, side_size, _ = x.shape
        m = nn.MaxPool2d((2, 2), stride=(2, 2))
        #batch_size, channel_size, side_size, _ = m(x).shape
        fm1, embedding1, conv5_b1 = self.pretrained_model1(m(x))
        # raw branch
        raw_logits = self.rawcls_net(embedding)
        raw_logits1 = self.rawcls_net(embedding1)
        raw_logits = (raw_logits1 + raw_logits)/2
        #SCDA
        fm = torch.cat((fm, fm1), 1)
        conv5_b = torch.cat((conv5_b, conv5_b1), 1)
        ...
        # t = local_imgs[0, ...]
        # t = t*0.5 + 0.5
        # plt.imshow(trans(t.cpu()))
        # plt.show()

        #local_fm, local_embeddings, _ = self.pretrained_model(x.detach())  # [N, 2048]
        #local_logits = self.rawcls_net(local_embeddings)  # [N, 200]
        proposalN_indices, proposalN_windows_scores, window_scores \
            = self.APPM(self.proposalN, fm.detach(), ratios, window_nums_sum, N_list, iou_threshs, DEVICE)
        ...
        return proposalN_windows_scores, proposalN_windows_logits, proposalN_indices, \
               window_scores, coordinates, raw_logits
