...
def train(model,
    ...
    for epoch in range(start_epoch + 1, end_epoch + 1):
        ...
        for i, data in enumerate(tqdm(trainloader)):
            ...
            window_scores, _, raw_logits = model(images, epoch, i, 'train')
            ...
            if epoch < 2:
                ...
                total_loss = raw_loss + windowscls_loss
        ...
        date = open("./models/train.txt", 'a')  # 鍐欏叆妯″紡

        date.write('epoch:%d' % epoch + '.pth' + ':' + 'raw accuracy: {:.2f}%'.format(
            100. * raw_accuracy))
        date.write("\n")
